...
def init(args):
    ...
    for file_index in range(args.num_jsons):
        path = f'./assets/jsons/challenge_{challenge}/generated_{file_index + 1}.json'
...
def challenge_1(unused_file_index):
    ...
    machine_ids = [
        f'mach::0{machine_id}' for machine_id in range(num_machines)
    ]
...
def challenge_3(file_index):
    ...
    lines = []
    with open(os.path.join(outputs_path, f'output_{file_index + 1}.txt')) as f:
        lines = f.read().split('\n')
